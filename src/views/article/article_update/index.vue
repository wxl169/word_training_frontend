<template>
    <div style=" padding: 0px">
        <a-card :bordered="false" style="width: 100%;">
            <template #title>
                <div class="container">
                    <router-link :to="{ path: '/user/release' }">
            ...
            item.tagName }}</a-select-option>
                    ...
                    <a-form-item :wrapper-col="{ offset: 8, span: 16 }">
                        <a-button html-type="submit" style="width: 200px;" @click="submitForm(3)">保存草稿</a-button>
...
import { getArticleUpdateById, updateArticle, uploadImage } from '@/api/article';
import type { ArticleUpdateRequest } from '@/api/article/type';
...
onMounted(async () => {
    ...
    const res = await getArticleUpdateById(IdRequest)
    if (res.code == 0) {
        formState.title = res.data?.title as string;
        valueHtml.value = res.data?.content as string;
        formState.description = res.data?.description as string;
        formState.tags = res.data?.tagList as string[];
        formState.coverImage = res.data?.coverImage as string;
        formState.permissions = res.data?.permissions as number;
        formState.articleId = res.data?.id as number;
        formState.status = res.data?.status as number;
    }
})
...
//表单数据
const formState: UnwrapRef<ArticleUpdateRequest> = reactive({
    articleId: 0,//文章id
    title: '',//标题
    content: '',//内容
    description: '',//描述
...
editorConfig.MENU_CONF['uploadImage'] = {
    async customUpload(file: string | Blob, insertFn: (arg0: string | undefined, arg1: string, arg2: string | undefined) => void) {
    ...
    formRef.value
        ...
        .then(() => {
            Modal.confirm({
                ...
                content: '当你点击确认修改文章后，文章将重新提交让管理员审核',
                async onOk() {
                    //获取富文本中的数据
                    formState.content = valueHtml.value;
                    formState.status = status;
                    const res = await updateArticle(formState)
